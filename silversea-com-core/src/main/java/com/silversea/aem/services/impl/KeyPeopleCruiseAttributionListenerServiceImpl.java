						 ...
						 if(val != null && val.getString().equals(WcmConstants.RT_KEY_PEOPLE)){
							...
							if(valueToCompare){
								...
								if(currentVal != null && previousVal != null){
									...
									Collection<String> toDelete = CollectionUtils.removeAll(previousValSet, currentValSet);
									Collection<String> toAdd = CollectionUtils.removeAll(currentValSet, previousValSet);
									Collection<String> toUpdate = CollectionUtils.intersection(currentValSet, previousValSet);
									...
									for (String toDel : toDelete) {
										...
										if(cruiseToModify != null){
											Node cruiseNode = cruiseToModify.getChild("jcr:content").adaptTo(Node.class);
											if(cruiseNode.hasProperty("reference")){
												Value[] keyPeopleList = cruiseNode.getProperty("reference").getValues();
