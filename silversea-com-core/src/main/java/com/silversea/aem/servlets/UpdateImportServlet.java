...
public class UpdateImportServlet extends SlingSafeMethodsServlet {
	...
	@Reference
	private ShipsUpdateImporter updateShips;
		...
		try {
			...
			if (!isAllRuning) {
				...
				if (all || mode.equals(Mode.ships)) {
					...
					watch.reset();
					watch.start();
					updateShips.updateImporData();
					nbrError = updateShips.updateImporData().getErrorNumber();
					nbrSucces = updateShips.updateImporData().getSuccesNumber();
