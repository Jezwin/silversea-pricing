...
public class CruisesItinerariesLandProgramsImporterImpl implements CruisesItinerariesLandProgramsImporter {
    ...
    public ImportResult importAllItems(final boolean update) throws ImporterException {
                            ...
                            if (itineraryModel.isItinerary(landProgramDiff.getVoyageId(), landProgramDiff.getDate().toGregorianCalendar(),
                                ...
                                try {
                                    ...
                                    if(!BooleanUtils.isTrue(landProgramDiff.getIsDeleted())){
	                                    // TODO to check : getLandItineraryId() is not unique over API
	                                    if (!landProgramsNode.hasNode(String.valueOf(landProgramDiff.getLandItineraryId()))) {
	                                    	//Should create
	                                        final Node landProgramNode = landProgramsNode.addNode(JcrUtil.createValidChildName(landProgramsNode,
	                                        ...
	                                        landProgramNode.setProperty("landProgramId", landProgramId);
	                                        landProgramNode.setProperty("landProgramItineraryId", landProgramDiff.getLandItineraryId());
	                                        landProgramNode.setProperty("date", landProgramDiff.getDate().toGregorianCalendar());
	                                        landProgramNode.setProperty("sling:resourceType", "silversea/silversea-com/components/subpages/itinerary/landprogram");
	
	                                    }else {
	                                    	//Should update
	                                    	final String lang = LanguageHelper.getLanguage(pageManager, itineraryResource);
	                                    	final Node landProgramNodeToUpdate = landProgramsNode.getNode(String.valueOf(landProgramDiff.getLandItineraryId()));
	                                    	
	                                    	landProgramNodeToUpdate.setProperty("date", landProgramDiff.getDate().toGregorianCalendar());
	                                    	
	                                    }
                                    }else {
                                    	//Should delete
                                    	final Node landProgramNodeToDelete = landProgramsNode.getNode(String.valueOf(landProgramDiff.getLandItineraryId()));
        ...
        LOGGER.info("Ending itineraries land programs import, success: {}, errors: {}, api calls : {} and {} for Diff",
