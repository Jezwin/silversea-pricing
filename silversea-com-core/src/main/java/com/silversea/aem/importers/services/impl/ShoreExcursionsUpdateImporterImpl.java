									...
									try {
										...
										Iterator<Resource> resources = resourceResolver
												.findResources("/jcr:root/content/silversea-com/" + loc
														+ "//element(*,cq:Page)[jcr:content/shorexId=\""
														+ shorex.getShorexId() + "\"]", "xpath");
										...
										if (resources.hasNext()) {
											...
											LOGGER.debug("Shorex page {} with ID {} already exists",
													shorex.getShorexName(), shorex.getShorexId());
										} else {
											...
											if (cityId != null) {
												...
												if (portsResources.hasNext()) {
													...
													if (!replicat
															.getReplicationStatus(session, excursionsPage.getPath())
															.isActivated()) {
														session.save();
													...
													excursionPage = pageManager.create(excursionsPage.getPath(),
															JcrUtil.createValidChildName(
																	excursionsPage.adaptTo(Node.class),
										...
										if (excursionPage != null && (BooleanUtils.isFalse(shorex.getIsDeleted())
											...
											excursionPageContentNode.setProperty("codeExcursion",
													shorex.getShorexCod());
											excursionPageContentNode.setProperty("apiLongDescription",
													shorex.getDescription());
