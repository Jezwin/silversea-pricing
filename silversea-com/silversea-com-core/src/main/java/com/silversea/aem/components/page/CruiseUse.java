...
public class CruiseUse extends EoHelper {
    ...
    public void activate() throws Exception {
        ...
        Map<String, List<SilverseaAsset>> mapAsset =
                mapAsset = AssetUtils.addAllShipAreaAssets(getResourceResolver(), cruiseModel.getShip().getSuites());
        virtualTourAssetsList = mapAsset.get("assetsVirtualTour");

        if (cruiseModel.getShip() != null &&
                StringUtils.isNotEmpty(cruiseModel.getShip().getPhotoVideoSuiteSelectionReference())) {
            suitesAssetsList = AssetUtils
                    .buildSilverseaAssetList(cruiseModel.getShip().getPhotoVideoSuiteSelectionReference(),
        ...
        for (ItineraryModel itinerary : cruiseModel.getCompactedItineraries()) {
            ...
            if (portModel != null && !"day-at-sea".equals(portModel.getName())) {
                boolean newPort = ports.add(portModel.getName());
                if (newPort) {
        ...
        for (PriceModel priceModel : cruiseModel.getPrices()) {
            if (priceModel.getGeomarket() != null && priceModel.getGeomarket().equals(geomarket)
                ...
                if (!added) {
                    prices.add(
                            new SuitePrice(priceModel.getSuite(), priceModel, locale, priceModel.getSuiteCategory()));
                }
     ...
     * based on ship
     *
     * @param shipName Ship name of the current cruise to show
     */
