 ...
 */
public class FindYourCruiseLandingUse extends AbstractGeolocationAwareUse {
    ...
    public void activate() throws Exception {
        ...
        String exclusiveOfferID = getProperties().get("eoId", String.class);
        String destinationID = getProperties().get("destinationId", String.class);
        String shipID = getProperties().get("shipId", String.class);
        String featureID = getProperties().get("featureId", String.class);


        // Get type from configuration
        Conf confRes = getResource().adaptTo(Conf.class);
        if (confRes != null) {
        ...
        if (!StringUtils.isEmpty(shipID)) {
            for (int i = 0; i < ships.size(); i++) {
                if (ships.get(i).getShipId().equals(shipID)) {
                    shipFilter = ships.get(i).getName();
                    prefilterByShip = true;
                }
            }
        }
        ...
        if (!StringUtils.isEmpty(featureID)) {
            features.forEach(i -> {
                if (i.getFeatureId().equals(featureID)) {
                    featuresFilter.add(i);
                    prefilterByFeature = true;
                }
            });
        }

        if (!StringUtils.isEmpty(exclusiveOfferID)) {
            exclusiveOfferFilter = exclusiveOfferID;
        }

        // Landing page adaptation : If properties is set in current component configuration - pre-filter available cruise with only voyage code listed
        String voyageCodeList = (String) getProperties().get("voyagecodelist");
        Map<String, Boolean> insertCodeCruise = new HashedMap();
        if (!Strings.isNullOrEmpty(voyageCodeList)) {
            List<CruiseModelLight> newAllCruises = new ArrayList<>();
            String[] parts = voyageCodeList.split(",");
            for (String vcode : parts) {
                vcode = vcode.trim();
                for (CruiseModelLight cruise : allCruises) {
                    if (cruise.getCruiseCode().equals(vcode) && (insertCodeCruise.get(vcode) == null)) {
                        newAllCruises.add(cruise);
                        insertCodeCruise.put(vcode, true);
                    }
                }
            }
        ...
        // Landing Page adaptation : If voyage is in the past - do not display them
        List<CruiseModelLight> newAllCruises = new ArrayList<>();
        for (CruiseModelLight cruise : allCruises) {
        ...
        allCruises = newAllCruises;
        Boolean noWaitlist = (getProperties().get("preFilterWaitlist") != null) ?
                getProperties().get("preFilterWaitlist", Boolean.class) : false;
        ...
        for (final CruiseModelLight cruise : allCruises) {
            ...
            try {
                int cruiseDuration = Integer.parseInt(cruise.getDuration());
            ...
            if (!exclusiveOfferFilter.equals(FILTER_ALL)) {
                ...
                for (final ExclusiveOfferModelLight exclusiveOffer : cruise.getExclusiveOffers()) {
                ...
                includeCruiseNotFilteredByDepartureDate =
                        includeCruiseNotFilteredByDepartureDate && exclusiveOfferInCruise;
            ...
            if (includeCruise) {
                ...
                if (!(noWaitlist && cruise.getLowestPrices().get(geomarket + currency) == null)) {
        ...
        for (final CruiseModelLight cruise : filteredCruises) {
            ...
            if (i >= (activePage - 1) * resultPerPage) {
                CruiseItem cruiseItem = new CruiseItem(cruise, geomarket, currency, locale);
                cruises.add(cruiseItem);
            }
